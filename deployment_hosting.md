...
6. *CI checks & migrations*

   - We will run tests on PRs (unit + integration where possible).
   - We will run DB migration scripts in CI using the supabase CLI or a migration runner before production deploys.

7. *Domain & HTTPS*

   - Domain, TLS/HTTPS is auto-managed by Vercel.

8. *Monitoring, rollback & backups*
